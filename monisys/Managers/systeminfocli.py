...
class SystemInfoCLI:
    ...
    def get_cpuinfo(self):
        ...
        return (address_width, cpu_status, current_clock_speed, device_id,
                logical_processors, manufacturer, max_clock_speed, model,
                number_of_cores, processor_type, socket_designation)
    ...
    def display_uptime(self):
        try:
            ...
            panel =  Panel("[bold]" + cpu_info_str + "[/bold]", title="CPU INFO", style="white", expand=False)
    ...
    def display_kernel_info(self):
        try:
            ...
            panel =  Panel("[bold]" + kernel_info_string + "[/bold]", title="Kernal Info", style="white", expand=False)
            self.console.print(panel)
        except KeyboardInterrupt:
            print("[*] Closed")
